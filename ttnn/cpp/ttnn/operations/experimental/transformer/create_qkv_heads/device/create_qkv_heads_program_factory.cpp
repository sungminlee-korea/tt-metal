...
namespace ttnn::operations::experimental::transformer {
    ...
    static inline operation::ProgramWithCallbacks create_heads_combined_qkv_sharded(const Tensor &input_tensor, const std::vector<uint32_t> &&heads_per_group, const uint32_t head_dim, const uint32_t groups, std::vector<Tensor> &output, bool transpose_k) {
        ...
        auto reader_kernel_id = tt_metal::CreateKernel(
            program,
            "ttnn/cpp/ttnn/operations/experimental/transformer/create_qkv_heads/device/kernels/reader_create_qkv_heads_sharded.cpp",
            all_cores,
        ...
        // qkv tensor
        auto c_in0_config = CircularBufferConfig(input_size, {{CB::cb_0, data_format}}).set_page_size(CB::cb_0, single_tile_size).set_globally_allocated_address(*input_tensor.buffer());
        auto cb_in0_id = CreateCircularBuffer(program, all_cores, c_in0_config);

        // q sharded
        auto c_out0_config = CircularBufferConfig(q_size, {{CB::cb_16, data_format}})
            .set_page_size(CB::cb_16, single_tile_size).set_globally_allocated_address(*output[0].buffer());
        auto cb_out0_id = CreateCircularBuffer( program, all_cores, c_out0_config );
        // k sharded
        auto c_out1_config = CircularBufferConfig(k_size, {{CB::cb_17, data_format}})
            .set_page_size(CB::cb_17, single_tile_size).set_globally_allocated_address(*output[1].buffer());
        auto cb_out1_id = CreateCircularBuffer( program, all_cores, c_out1_config );
        // v sharded
        auto c_out2_config = CircularBufferConfig(v_size, {{CB::cb_18, data_format}})
            .set_page_size(CB::cb_18, single_tile_size).set_globally_allocated_address(*output[2].buffer());
        auto cb_out2_id = CreateCircularBuffer( program, all_cores, c_out2_config );

        if (transpose_k) {
            auto c_im0_config = CircularBufferConfig(k_size, {{CB::cb_24, data_format}})
                .set_page_size(CB::cb_24, single_tile_size);
