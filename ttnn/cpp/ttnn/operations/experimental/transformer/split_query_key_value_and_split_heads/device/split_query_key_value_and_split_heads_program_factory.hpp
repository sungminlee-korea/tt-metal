...
operation::ProgramWithCallbacks multi_core_split_query_key_value_and_split_heads_sharded(const Tensor &a, std::vector<Tensor>& output, CoreCoord compute_with_storage_grid_size) {
    ...
    // Create circular buffers
    // in0 sharded
    auto c_in0_config = CircularBufferConfig(in0_CB_size, {{CB::cb_0, cb_data_format}})
        .set_page_size(CB::cb_0, single_tile_size).set_globally_allocated_address(*a.buffer());
    auto cb_in0_id = CreateCircularBuffer(program, all_cores, c_in0_config);
    // im
    auto c_im0_config = CircularBufferConfig(im0_CB_size, {{CB::cb_24, cb_data_format}})
        .set_page_size(CB::cb_24, single_tile_size);
    auto cb_im0_id = CreateCircularBuffer(program, all_cores, c_im0_config);
    // q sharded
    auto c_out0_config = CircularBufferConfig(out_CB_size, {{CB::cb_16, cb_data_format}})
    ...
    // k sharded
    auto c_out1_config = CircularBufferConfig(out_CB_size, {{CB::cb_17, cb_data_format}})
    ...
    // v sharded
    auto c_out2_config = CircularBufferConfig(out_CB_size, {{CB::cb_18, cb_data_format}})
