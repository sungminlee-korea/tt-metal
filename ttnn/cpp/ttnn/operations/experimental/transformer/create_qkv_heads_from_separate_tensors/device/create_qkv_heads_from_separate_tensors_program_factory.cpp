...
namespace ttnn::operations::experimental::transformer {
    ...
    static inline operation::ProgramWithCallbacks create_qkv_separate(const Tensor &input_tensor_q, const Tensor &input_tensor_kv, const uint32_t num_q_heads, const uint32_t num_kv_heads, const uint32_t head_dim, std::vector<Tensor> &output, bool transpose_k) {
        ...
        {{CB::cb_0, q_data_format}}).set_page_size(CB::cb_0, single_tile_size).set_globally_allocated_address(*input_tensor_q.buffer());
        auto cb_in0_id = CreateCircularBuffer(program, all_cores, c_in0_config);

        auto c_in1_config = CircularBufferConfig(kv_size, {
            {CB::cb_1, kv_data_format}}
            ).set_page_size(CB::cb_1, single_tile_size).set_globally_allocated_address(*input_tensor_kv.buffer());
        auto cb_in1_id = CreateCircularBuffer(program, all_cores, c_in1_config);

        // q sharded
        auto c_out0_config = CircularBufferConfig(q_size, {{CB::cb_16, q_data_format}})
        ...
        // k sharded
        auto c_out1_config = CircularBufferConfig(k_size, {{CB::cb_17, kv_data_format}})
        ...
        // v sharded
        auto c_out2_config = CircularBufferConfig(v_size, {{CB::cb_18, kv_data_format}})
        ...
        if (transpose_k) {
            auto c_im0_config = CircularBufferConfig(k_size, {{CB::cb_24, kv_data_format}})
