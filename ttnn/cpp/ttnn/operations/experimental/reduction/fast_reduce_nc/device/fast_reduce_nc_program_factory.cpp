...
operation::ProgramWithCallbacks reduce_nc_factory(const ttnn::Tensor &input, const ttnn::Tensor &output, int64_t dim,const ttnn::DeviceComputeKernelConfig &compute_kernel_config) {
    ...
    tt_metal::CircularBufferConfig cb_scr0_config =
        tt_metal::CircularBufferConfig(in0_t*single_tile_size, {{CB::cb_0, cb_data_format}})
            .set_page_size(CB::cb_0, single_tile_size);
    auto cb_scr0 = tt_metal::CreateCircularBuffer(program, all_cores, cb_scr0_config);

    tt_metal::CircularBufferConfig cb_scr1_config =
        tt_metal::CircularBufferConfig(in1_t*cb_1_tile_size, {{CB::cb_1, cb_1_data_format}})
            .set_page_size(CB::cb_1, cb_1_tile_size);
    ...
    tt_metal::CircularBufferConfig cb_intermed0_config =
        tt_metal::CircularBufferConfig(intermed0_t*intermed_cb_single_tile_size, {{CB::cb_24, intermed_cb_data_format}})
            .set_page_size(CB::cb_24, intermed_cb_single_tile_size);
    auto cb_intermed0 = tt_metal::CreateCircularBuffer(program, all_cores, cb_intermed0_config);

    tt_metal::CircularBufferConfig cb_output_config =
        tt_metal::CircularBufferConfig(out0_t*single_tile_size, {{CB::cb_16, cb_data_format}})
            .set_page_size(CB::cb_16, single_tile_size);
