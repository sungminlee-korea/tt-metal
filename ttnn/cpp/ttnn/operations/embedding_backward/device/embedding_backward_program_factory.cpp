...
operation::ProgramWithCallbacks embedding_backward_multi_core(
    ...
    create_cb(CB::cb_0, program, all_cores, grad_single_tile_size, max_tiles_per_core, grad_cb_data_format);

    // To store index values for a single tile
    create_cb(CB::cb_1, program, all_cores, index_single_page_size, 1, index_cb_data_format);

    // To read from output tensor
    create_cb(CB::cb_2, program, all_cores, output_single_tile_size, max_tiles_per_core, output_cb_data_format);

    // To store mask values for a single tile
    create_cb(CB::cb_24, program, all_cores, mask_single_page_size, 1, mask_cb_data_format);

    // L1 scratch space to pass chunk_count from reader to UNPACK
    create_cb(
        CB::cb_25, program, all_cores, 16, 1, grad_cb_data_format);  // grad_cb_data_format doesn't matter here

    // For tiles to be written to the output
    create_cb(CB::cb_16, program, all_cores, output_single_tile_size, max_tiles_per_core, output_cb_data_format);
