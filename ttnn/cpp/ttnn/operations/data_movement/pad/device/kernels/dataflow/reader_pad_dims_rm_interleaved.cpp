...
void kernel_main() {
    ...
    constexpr uint32_t cb_id = tt::CB::cb_0;
    ...
    const uint32_t l1_addr_align_offset = 32 - l1_addr_partial % 32;  // NOTE: this is fine with double buffering since offset will be same for each page
    ...
    // #else
        const InterleavedAddrGen<src0_is_dram> s0 = {
            .bank_base_address = src_addr,
            .page_size = full_unpadded_X_nbytes
        };
    // #endif
        ...
        for (uint32_t z = 0; z < num_total_Z; ++ z) {
            for (uint32_t y = 0; y < num_local_Y; ++ y) {
                ...
                if (y >= num_local_unpadded_Y || z >= num_unpadded_Z || w >= num_unpadded_W) {
                    ...
                    fill_with_val_async(const_buffer_noc_addr, l1_addr, l1_addr, padded_X_nbytes, pad_value_const_buffer_nbytes, pad_value);
                } else {
