...
operation::ProgramWithCallbacks layernorm_multi_core(
    ...
    CircularBufferConfig cb_src0_config = CircularBufferConfig(in0_t*in_single_tile_size, {{tt::CB::cb_0, in_data_format}}).set_page_size(tt::CB::cb_0, in_single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_src0_config );
    CircularBufferConfig cb_out0_config = CircularBufferConfig(out0_t*out_single_tile_size, {{tt::CB::cb_16, out_data_format}}).set_page_size(tt::CB::cb_16, out_single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_out0_config );
    if (!rms_norm) {
        CircularBufferConfig cb_intermed1_config = CircularBufferConfig(im1_t*single_tile_size, {{tt::CB::cb_25, cb_data_format}}).set_page_size(tt::CB::cb_25, single_tile_size);
        CreateCircularBuffer( program, all_cores,  cb_intermed1_config );
    }
    CircularBufferConfig cb_in2_config = CircularBufferConfig(in2_t*bfloat16_tile_size, {{tt::CB::cb_2, tt::DataFormat::Float16_b}}).set_page_size(tt::CB::cb_2, bfloat16_tile_size);
    CreateCircularBuffer( program, all_cores, cb_in2_config );
    CircularBufferConfig cb_in3_config = CircularBufferConfig(in3_t*bfloat16_tile_size, {{tt::CB::cb_3, tt::DataFormat::Float16_b}}).set_page_size(tt::CB::cb_3, bfloat16_tile_size);
    CreateCircularBuffer( program, all_cores, cb_in3_config );
    CircularBufferConfig cb_intermed2_config = CircularBufferConfig(im2_t*single_tile_size, {{tt::CB::cb_26, cb_data_format}}).set_page_size(tt::CB::cb_26, single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_intermed2_config );
    if (!(rms_norm && !b.has_value())) {
        CircularBufferConfig cb_intermed0_config = CircularBufferConfig(im0_t*single_tile_size, {{tt::CB::cb_24, cb_data_format}}).set_page_size(tt::CB::cb_24, single_tile_size);
        CreateCircularBuffer( program, all_cores, cb_intermed0_config );
    }
    CircularBufferConfig c_intermed3_config = CircularBufferConfig(im3_t*single_tile_size, {{tt::CB::cb_27, cb_data_format}}).set_page_size(tt::CB::cb_27, single_tile_size);
    CreateCircularBuffer( program, all_cores, c_intermed3_config );
    CircularBufferConfig c_intermed4_config = CircularBufferConfig(im4_t*single_tile_size, {{tt::CB::cb_28, cb_data_format}}).set_page_size(tt::CB::cb_28, single_tile_size);
    CreateCircularBuffer( program, all_cores, c_intermed4_config );
    if (gamma.has_value() || beta.has_value()) {
        CircularBufferConfig c_intermed5_config = CircularBufferConfig(im5_t*single_tile_size, {{tt::CB::cb_29, cb_data_format}}).set_page_size(tt::CB::cb_29, single_tile_size);
        CreateCircularBuffer( program, all_cores, c_intermed5_config );
    }
    if (gamma.has_value()) {
        CircularBufferConfig c_in5_config = CircularBufferConfig(in5_t * gamma_single_tile_size, {{tt::CB::cb_5, gamma_cb_data_format}})
            .set_page_size(tt::CB::cb_5, gamma_single_tile_size);
        CreateCircularBuffer( program, all_cores, c_in5_config );
    }
    if (beta.has_value()) {
        CircularBufferConfig c_in6_config = CircularBufferConfig(in6_t * beta_single_tile_size, {{tt::CB::cb_6, beta_cb_data_format}})
            .set_page_size(tt::CB::cb_6, beta_single_tile_size);
    ...
    if (b) {
        ...
        // b is buffered into cb_1
        if (!rms_norm) {
            CircularBufferConfig c_intermed6_config = CircularBufferConfig(im6_t*single_tile_size, {{tt::CB::cb_30, cb_data_format}}).set_page_size(tt::CB::cb_30, single_tile_size);
            CreateCircularBuffer( program, all_cores, c_intermed6_config );
        }
        // cb_1 is input buffer for b
        CircularBufferConfig c_in1_config = CircularBufferConfig(in1_t*inb_single_tile_size, {{tt::CB::cb_1, inb_data_format}}).set_page_size(tt::CB::cb_1, inb_single_tile_size);
...
operation::ProgramWithCallbacks layernorm_multi_core_sharded(
    ...
    // Create circular buffers
    // in0 sharded
    uint32_t in0_cb_index = tt::CB::cb_0;
    ...
    // in1 sharded
    uint32_t in1_cb_index = tt::CB::cb_1;
    CBHandle cb_in1 = 0;
    if (b) {
    ...
    // in2 scaler
    uint32_t in2_cb_index = tt::CB::cb_2;
    ...
    // in4 scaler-c
    uint32_t in4_cb_index = tt::CB::cb_4;
    ...
    // in3 eps
    uint32_t in3_cb_index = tt::CB::cb_3;
    ...
    // gamma
    if (gamma.has_value()) {
        uint32_t in5_cb_index = tt::CB::cb_5;
    ...
    // beta
    if (beta.has_value()) {
        uint32_t in6_cb_index = tt::CB::cb_6;
    ...
    // ex_partial
    if(!rms_norm) {
        uint32_t ex_cb_partial_index = tt::CB::cb_8;
        ...
        // ex
        uint32_t ex_cb_index = tt::CB::cb_9;
        ...
        // ex_external
        uint32_t ex_cb_external_index = tt::CB::cb_10;
    ...
    // ex_partial2
    uint32_t ex_cb_partial2_index = tt::CB::cb_11;
    ...
    // ex2
    uint32_t ex2_cb_index = tt::CB::cb_12;
    ...
    // ex_external2
    uint32_t ex_cb_external2_index = tt::CB::cb_13;
    ...
    // ex_global
    uint32_t ex_global_cb_index = tt::CB::cb_15;
