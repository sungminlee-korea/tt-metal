...
operation::ProgramWithCallbacks groupnorm_multi_core_sharded(
    ...
    tt::DataFormat in_mask_cb_data_format = input_mask.has_value() ? tt::tt_metal::datatype_to_dataformat_converter(input_mask.value().get_dtype()) : tt::DataFormat::Float16_b;
    ...
    std::string writer_kernel = "ttnn/cpp/ttnn/operations/normalization/groupnorm/device/kernels/dataflow/writer_unary_sharded_gn_rm_gb_v2.cpp";
    ...
    auto mcast_sender_compute_kernels_id = CreateKernel(
        ...
        mcast_sender_cores,
        tt::tt_metal::ComputeConfig{.math_fidelity = fidelity, .fp32_dest_acc_en = fp32_dest_acc_en, .math_approx_mode = math_approx_mode, .compile_args = mcast_sender_compute_compile_time_args, .defines = eltwise_binary_defines}
    );
    ...
    uint32_t output_cb_index = tt::CB::cb_16;
    CBHandle cb_in0;
    CBHandle cb_output;
    if (inplace) {
        ...
        CircularBufferConfig in0_out0_cb_config = tt::tt_metal::CircularBufferConfig(in0_CB_size, in0_out0_cb_data_format_spec)
    ...
    uint32_t in_cb_index = tt::CB::cb_29;
    ...
    if (untilize_out) {
        uint32_t out_cb_index = tt::CB::cb_30;
    ...
    // input mask
    if (input_mask.has_value()) {
        uint32_t in_mask_cb_index = tt::CB::cb_28;
    ...
    if (reader_repack_output) {
        uint32_t repack_cb_index = tt::CB::cb_26;
        uint32_t repack_out_cb_index = tt::CB::cb_31;
        std::map<uint8_t, tt::DataFormat> in0_out0_cb_data_format_spec {
            {repack_cb_index, in_data_format},
            {repack_out_cb_index, in_data_format}
        };
        tt::tt_metal::CircularBufferConfig repack_cb_config = tt::tt_metal::CircularBufferConfig(repack_CB_size, in0_out0_cb_data_format_spec)
    ...
    // x
    uint32_t x_cb_index = tt::CB::cb_24;
    ...
    // xmm
    uint32_t xmm_cb_index = tt::CB::cb_25;
