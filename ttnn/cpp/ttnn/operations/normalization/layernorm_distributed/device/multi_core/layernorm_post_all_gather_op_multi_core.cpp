...
operation::ProgramWithCallbacks layernorm_post_allgather_multi_core(
    ...
    */
    ...
    // cb_0 -> a
    CircularBufferConfig cb_src0_config = CircularBufferConfig(in0_tiles*in_single_tile_size, {{tt::CB::cb_0, in_data_format}}).set_page_size(tt::CB::cb_0, in_single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_src0_config );
    // cb_1 -> stats
    CircularBufferConfig cb_stats_config = CircularBufferConfig(in1_tiles*stats_single_tile_size, {{tt::CB::cb_1, stats_data_format}}).set_page_size(tt::CB::cb_1, stats_single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_stats_config );
    // cb_2 -> gamma
    if (gamma.has_value()) {
        CircularBufferConfig cb_gamma_config = CircularBufferConfig(in2_tiles*gamma_single_tile_size, {{tt::CB::cb_2, gamma_cb_data_format}}).set_page_size(tt::CB::cb_2, gamma_single_tile_size);
        CreateCircularBuffer( program, all_cores, cb_gamma_config );
    }
    // cb_3 -> beta
    if (beta.has_value()) {
        CircularBufferConfig cb_beta_config = CircularBufferConfig(in3_tiles*beta_single_tile_size, {{tt::CB::cb_3, beta_cb_data_format}}).set_page_size(tt::CB::cb_3, beta_single_tile_size);
        CreateCircularBuffer( program, all_cores, cb_beta_config );
    }
    // cb_4 -> epsilon
    CircularBufferConfig cb_eps_config = CircularBufferConfig(in4_tiles*bfloat16_tile_size, {{tt::CB::cb_4, tt::DataFormat::Float16_b}}).set_page_size(tt::CB::cb_4, bfloat16_tile_size);
    CreateCircularBuffer( program, all_cores, cb_eps_config );
    // cb_5 -> reduce scalar
    CircularBufferConfig cb_reduce_config = CircularBufferConfig(in5_tiles*bfloat16_tile_size, {{tt::CB::cb_5, tt::DataFormat::Float16_b}}).set_page_size(tt::CB::cb_5, bfloat16_tile_size);
    CreateCircularBuffer( program, all_cores, cb_reduce_config );

    // LN and RMS shared intermediates //
    // cb_24 -> [mean(x**2), mean(x)]
    CircularBufferConfig cb_intermed0_config = CircularBufferConfig(intermed0_tiles*single_tile_size, {{tt::CB::cb_24, cb_data_format}}).set_page_size(tt::CB::cb_24, single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_intermed0_config );
    // cb_26 -> var = mean(x**2) - mean(x)**2
    CircularBufferConfig cb_intermed2_config = CircularBufferConfig(intermed2_tiles*single_tile_size, {{tt::CB::cb_26, cb_data_format}}).set_page_size(tt::CB::cb_26, single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_intermed2_config );
    // cb_27 -> var + epsilon
    CircularBufferConfig cb_intermed3_config = CircularBufferConfig(intermed3_tiles*single_tile_size, {{tt::CB::cb_27, cb_data_format}}).set_page_size(tt::CB::cb_27, single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_intermed3_config );
    // cb_28 -> 1/sqrt(var + epsilon)
    CircularBufferConfig cb_intermed4_config = CircularBufferConfig(intermed4_tiles*single_tile_size, {{tt::CB::cb_28, cb_data_format}}).set_page_size(tt::CB::cb_28, single_tile_size);
    CreateCircularBuffer( program, all_cores, cb_intermed4_config );
    // cb_30 -> (x - mean(x)) * 1/sqrt(var + epsilon)
    CircularBufferConfig cb_intermed6_config = CircularBufferConfig(intermed6_tiles*single_tile_size, {{tt::CB::cb_30, cb_data_format}}).set_page_size(tt::CB::cb_30, single_tile_size);
    ...
    if (!is_rmsnorm) {
        // cb_25 -> mean(x)**2
        CircularBufferConfig cb_intermed1_config = CircularBufferConfig(intermed1_tiles*single_tile_size, {{tt::CB::cb_25, cb_data_format}}).set_page_size(tt::CB::cb_25, single_tile_size);
        CreateCircularBuffer( program, all_cores, cb_intermed1_config );
        // cb_29 -> x - mean(x)
        CircularBufferConfig cb_intermed5_config = CircularBufferConfig(intermed5_tiles*single_tile_size, {{tt::CB::cb_29, cb_data_format}}).set_page_size(tt::CB::cb_29, single_tile_size);
        ...
        if (beta.has_value()) {
            ...
            // cb_31 -> (x - mean(x)) * 1/sqrt(var + epsilon) * gamma
            CircularBufferConfig cb_intermed7_config = CircularBufferConfig(intermed7_tiles*single_tile_size, {{tt::CB::cb_31, cb_data_format}}).set_page_size(tt::CB::cb_31, single_tile_size);
            CreateCircularBuffer( program, all_cores, cb_intermed7_config );
        }
    }


    CircularBufferConfig cb_out0_config = CircularBufferConfig(out0_tiles*out_single_tile_size, {{tt::CB::cb_16, out_data_format}}).set_page_size(tt::CB::cb_16, out_single_tile_size);
