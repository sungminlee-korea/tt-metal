...
void MorehCumsumDeviceOperation::validate_inputs(
    ...
    for (int i = 0; i < input_shape.rank(); ++i) {
        TT_FATAL(input_shape[i] == output_shape[i], "Input shape must match output shape. Received input_shape = {} and output_shape = {}.", input_shape[i], output_shape[i]);
        TT_FATAL(input_shape_wo_padding[i] == output_shape_wo_padding[i], "Input and output shapes (excluding padding) must be equal. Received input_shape_wo_padding = {} and output_shape_wo_padding = {}.", input_shape_wo_padding[i], output_shape_wo_padding[i]);
    }
}
