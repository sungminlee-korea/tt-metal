...
void MAIN {
    ...
    binary_op_init_common(tt::CB::cb_0, tt::CB::cb_24, tt::CB::cb_16);
    ...
    for (uint32_t t = 0; t < num_tiles; t++) {
        ...
        for(uint32_t tile_index = 0; tile_index < per_core_block_dim; ++tile_index) {
            ...
            if (once)
            {
                cb_reserve_back(tt::CB::cb_24, 1);
                tile_regs_acquire();
                copy_tile_to_dst_init_short();
                copy_tile(tt::CB::cb_0, 0, 0); // copy from c_in[0] to DST[0]
                tile_regs_commit();
                tile_regs_wait();
                ...
                else
                {
                    ...
                    cb_push_back(tt::CB::cb_24, 1);
                }
                tile_regs_release();
            }else {
                tile_regs_acquire();
                mul_tiles_init();
                mul_tiles(tt::CB::cb_0, tt::CB::cb_24, 0, 0, 0);
                tile_regs_commit();
                tile_regs_wait();
                if (last_tile)
                {
