...
operation::ProgramWithCallbacks moreh_norm_w_impl(const Tensor &input, float p, const Tensor &output, const ttnn::DeviceComputeKernelConfig compute_kernel_config) {
    ...
    const auto reader_kernel_file =
        "ttnn/cpp/ttnn/deprecated/tt_dnn/op_library/moreh_norm/moreh_norm_w/kernels/"
    ...
    for (uint32_t i = 0, tile_offset = 0; i < num_cores_to_be_used; ++i) {
        ...
        const std::vector<uint32_t> writer_runtime_args{
            output_addr, static_cast<uint32_t>(is_dram(output)), num_units_per_core, Wt, tile_offset};
