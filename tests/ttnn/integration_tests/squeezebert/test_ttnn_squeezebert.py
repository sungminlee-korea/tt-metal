...
def test_squeezebert_attention(mesh_device, model_name, batch_size, sequence_size, torch_dtype, reset_seeds):
    ...
    parameters = ttnn_functional_squeezebert.custom_preprocessor(state_dict, weights_mesh_mapper, mesh_device)
...
def test_squeezebert_for_question_answering(mesh_device, model_name, batch_size, sequence_size, reset_seeds):
    ...
    assert_with_pcc(torch_output.start_logits, tt_start_logits, 0.9479)
    assert_with_pcc(torch_output.end_logits, tt_end_logits, 0.8779)
