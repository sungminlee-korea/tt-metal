...
run_perf_models_other() {
    ...
    if [ "$tt_arch" == "wormhole_b0" ]; then
        ...
        env WH_ARCH_YAML=wormhole_b0_80_arch_eth_dispatch.yaml pytest models/experimental/functional_mnist/tests/test_perf_mnist.py -m $test_marker
    fi
...
run_device_perf_models() {
    ...
    if [ "$tt_arch" == "wormhole_b0" ]; then
        ...
        env WH_ARCH_YAML=wormhole_b0_80_arch_eth_dispatch.yam pytets models/demos/functional_mnist/tests/test_perf_mnist.py::test_performance_mnist -m $test_marker
    fi
